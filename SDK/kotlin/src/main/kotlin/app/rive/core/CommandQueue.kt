...
class CommandQueue(
    ...
    /**
     * Draw multiple artboards in a single batch on a shared surface.
     *
     * This renders all items into one GPU pass, reducing per-frame context switches from N to 1.
     * Each item is drawn at its specified viewport position within the shared surface.
     *
     * @param surface The shared surface to draw all items into.
     * @param artboardHandles Array of artboard handles for each item.
     * @param stateMachineHandles Array of state machine handles for each item.
     * @param viewportXs Array of x-positions (relative to surface) for each item.
     * @param viewportYs Array of y-positions (relative to surface) for each item.
     * @param viewportWidths Array of widths for each item's viewport.
     * @param viewportHeights Array of heights for each item's viewport.
     * @param fits Array of fit mode native mappings for each item.
     * @param alignments Array of alignment native mappings for each item.
     * @param scaleFactors Array of scale factors for each item.
     * @param clearColors Array of per-item clear colors in AARRGGBB format.
     * @param surfaceClearColor The color to clear the entire surface with before drawing.
     * @throws IllegalStateException If the CommandQueue has been released.
     */
    @Throws(IllegalStateException::class)
    fun drawBatch(
