...
fun Rive(
    ...
    LaunchedEffect(stateMachineHandle, viewModelInstance) {
        if (viewModelInstance == null) {
            ...
            android.util.Log.d("Rive/DEBUG", "[BIND] No VMI to bind for sm=$stateMachineHandle")
            return@LaunchedEffect
        }

        android.util.Log.d("Rive/DEBUG", "[BIND] Binding VMI=${viewModelInstance.instanceHandle} to SM=$stateMachineHandle")
        ...
        isSettled = false
        android.util.Log.d("Rive/DEBUG", "[BIND] isSettled=false after bind")

        // Subscribe to the instance's dirty flow to unsettle when properties change
        viewModelInstance.dirtyFlow.collect {
            android.util.Log.d("Rive/DEBUG", "[DIRTY] dirtyFlow fired → isSettled=false (was=$isSettled)")
    ...
    LaunchedEffect(stateMachineHandle) {
        riveWorker.settledFlow
            ...
            .collect {
                android.util.Log.d("Rive/DEBUG", "[SETTLED] SM=$stateMachineHandle settled → isSettled=true")
    ...
    LaunchedEffect(
        ...
        if (surface == null) {
            android.util.Log.d("Rive/DEBUG", "[DRAW] Surface is null, skipping")
            RiveLog.d(DRAW_TAG) { "Surface is null, skipping drawing" }
            return@LaunchedEffect
        }
        if (!playing) {
            android.util.Log.d("Rive/DEBUG", "[DRAW] playing=false, drawing once")
            RiveLog.d(DRAW_TAG) {
        ...
        lifecycleOwner.lifecycle.repeatOnLifecycle(Lifecycle.State.RESUMED) {
            android.util.Log.d("Rive/DEBUG", "[DRAW] RESUMED — entering frame loop, artboard=$artboardHandle, sm=$stateMachineHandle, surface=${surfaceWidth}x${surfaceHeight}")
            ...
            while (isActive) {
                ...
                val deltaTime = if (lastFrameTime == 0L) {
                    ...
                    (frameTimeNs - lastFrameTime).nanoseconds
                }
                ...
                if (isSettled) {
                    if (frameCount <= 5 || frameCount % 120 == 0) {
                        android.util.Log.d("Rive/DEBUG", "[DRAW] frame#$frameCount SKIPPED (settled)")
                    }
                    continue
                }

                if (frameCount <= 5 || frameCount % 60 == 0) {
                    android.util.Log.d("Rive/DEBUG", "[DRAW] frame#$frameCount DRAWING (dt=$deltaTime)")
                }
                ...
                // Skip advancing on the very first frame (dt=0).
                // The Rive runtime consumes triggers during advance(0)
                // without processing state transitions, causing one-shot
                // triggers to be silently lost.
                if (deltaTime > kotlin.time.Duration.ZERO) {
                ...
                TextureView(context).apply {
                    ...
                    surfaceTextureListener = object : TextureView.SurfaceTextureListener {
                        ...
                        override fun onSurfaceTextureAvailable(
                            ...
                            android.util.Log.d("Rive/DEBUG", "[SURFACE] available ${width}x${height}")
                        ...
                        override fun onSurfaceTextureUpdated(surfaceTexture: SurfaceTexture) {
                            textureUpdateCount++
                            if (textureUpdateCount <= 5 || textureUpdateCount % 60 == 0) {
                                android.util.Log.d("Rive/DEBUG", "[TEXTURE_UPDATE] frame#$textureUpdateCount — TextureView received new frame")
                            }
